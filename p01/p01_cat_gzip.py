...
class CatTar(CommandOp):
    ...
    def create_dir(self):
        ...
        for lane, name in self.conf_obj.lane_user_dict.iteritems():
            if self.conf_obj.perm_dict[lane].startswith('0'):
                continue
    ...
    def calcpath(self):
        ...
        logging.debug("here is the list of files command: "+\
                      "ls "+self.s_what+"[1-9]_*_qseq.txt")
        ...
        for file in self.conf_obj.copy_to_users:
            for name in self.conf_obj.lane_user_dict.itervalues():
                ...
                if file.split('/')[-1] == 'Summary.htm':
                    ...
                    #TODO: raghu. check for error
                    self.move('Summary.htm', self.runid+'_Summary.htm')
                    os.chdir('..')

        os.chdir(self.runid_dir)
        self.remove(self.runid+"summary")
    ...
    def ftp_files(self):
        ...
        for lane, name_folder in self.conf_obj.ftp_user_dict.\
                iteritems():
            if self.conf_obj.perm_dict[lane].endswith('0'):
                continue
            os.chdir(self.runid_dir)
...
class Main:
    ...
    def check(self):
        ...
        if result.strip("\n").split("/")[-1] == "finished.txt":
            self.main()
            self.flag = 0
            os.chdir(self.conf_obj.backup_path)
